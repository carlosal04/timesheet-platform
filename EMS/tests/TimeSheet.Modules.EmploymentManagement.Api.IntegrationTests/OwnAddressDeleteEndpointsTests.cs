...
public sealed class OwnAddressDeleteEndpointsTests : IClassFixture<AuthApiFactory>
{
    private const string AuthCookieName = "ems.auth";
    private readonly AuthApiFactory _factory;

    public OwnAddressDeleteEndpointsTests(AuthApiFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task DeleteMyAddress_SoftDeletesOwnAddress()
    {
    ...
    [Fact]
    public async Task DeleteMyAddress_WhenPrimary_PromotesOldestReplacement()
    {
        await _factory.ResetDatabaseAsync();
        ...
        await _factory.ExecuteScopedAsync(async services =>
        {
            ...
            Assert.Contains(
                dbContext.AuditLogs,
                log => log.ActionType == AuditActionTypes.AddressSoftDeleted
                    && log.EntityId == deletedPrimaryId
                    && log.Result == AuditResults.Success);
            Assert.Contains(
                dbContext.AuditLogs,
                log => log.ActionType == AuditActionTypes.AddressPrimaryChanged
                    && log.EntityId == promotedId
                    && log.Result == AuditResults.Success);
        });
    }

    [Fact]
    public async Task DeleteMyAddress_ForAddressOwnedByAnotherEmployee_ReturnsForbidden()
    {
        await _factory.ResetDatabaseAsync();
    ...
    [Fact]
    public async Task DeleteMyAddress_ForAlreadyDeletedOwnAddress_ReturnsConflict()
    {
        await _factory.ResetDatabaseAsync();
